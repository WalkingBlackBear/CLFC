...
// debug_print ("WARN: ", $_REQUEST, basename(__FILE__).' LINE '.__LINE__);
...
// echo "<pre>".print_r($transaction_data,true)."</pre>";
        display_page_footer ();
      }
    elseif ($method == 'ajax')
      {
...
if ($type == 'edit')
  {
    ...
    $display = '
        <tr id="edit_dialog_'.$transaction_data['transaction_id'].'">
          <td colspan="10">
            <form class="edit_dialog" name="edit_dialog">
              <fieldset class="left">
                ...
                <input type="text" class="regular_input" name="text_key" id="text_key" value="'.$transaction_data['text_key'].'" placeholder="'.$transaction_data['text_key'].'" list="text_key_list" required>
              ...
              <fieldset class="right">
                ...
                <br>
                <label for="delcode_id_'.$transaction_data['transaction_id'].'">Delivery Code ID:</label>
                <input type="text" class="regular_input" name="delcode_id" id="delcode_id_'.$transaction_data['transaction_id'].'" list="delcode_list" value="'.$transaction_data['delcode_id'].'" placeholder="'.$transaction_data['delcode_id'].'">
                <datalist id="delcode_list">
                  '.get_delcode_list().'
                </datalist>
                <br>
...
debug_print ("WARN: X-1-X ", $_REQUEST, basename(__FILE__).' LINE '.__LINE__);
    ...
    $new_delcode_id = $_REQUEST['delcode_id'];
...
debug_print ("WARN: X-2-X ZERO SPLIT:", $zero_split, basename(__FILE__).' LINE '.__LINE__);
        $payment_transaction_id = add_to_ledger (array (
          ...
          'delcode_id' => $original_transaction_data['delcode_id'],
...
if ($type == 'new')
  {
    ...
    if (strlen ($transaction_data['delcode_id']) > 0)
      {
        $where .= '
          OR delcode_id = "'.$transaction_data['delcode'].'"';
      }
    // If the transaction is replaced, then abort and do not allow editing
          ...
          <td colspan="10">
            <form class="edit_dialog" name="edit_dialog">
              <fieldset class="left">
                ...
                <input type="text" name="source_key" id="source_key_'.$transaction_data['transaction_id'].'" value="'.$transaction_data['source_key'].'">
                <br>
                <label for="target_key_'.$transaction_data['transaction_id'].'">To:</label>
                <select name="target_type" id="target_type_'.$transaction_data['transaction_id'].'">
                  <option value="member"'.($transaction_data['target_type'] == 'member' ? ' selected' : '').'>member</option>
                ...
                <input type="text" name="target_key" id="target_key_'.$transaction_data['transaction_id'].'" value="'.$transaction_data['target_key'].'">
                <br>
              ...
              <fieldset class="right">
                ...
                <br>
                <label for="delcode_id">Delivery Code ID:</label>
                <input type="text" class="regular_input" name="delcode_id" id="delcode_id" list="delcode_list" value="" placeholder="'.$transaction_data['delcode_id'].'">
                <datalist id="delcode_list">
                  '.get_delcode_list().'
                </datalist>
                <br>
...
if ($type == 'add')
  {
    ...
    $original_transaction_id = $target;
    // Source
    list ($source_spec, $null) = explode (':', $_REQUEST['source_spec']);
    $new_source_type = preg_replace('/[0-9]/','',$source_spec);
    $new_source_key = preg_replace('/[^0-9]/','',$source_spec);
    // Target
    list ($target_spec, $null) = explode (':', $_REQUEST['target_spec']);
    $new_target_type = preg_replace('/[0-9]/','',$target_spec);
    $new_target_key = preg_replace('/[^0-9]/','',$target_spec);
    // Amount
    $new_amount = $_REQUEST['amount'];
    // Effective Date/time
    $new_effective_datetime = $_REQUEST['effective_datetime'];
...
function get_transaction_row ($transaction_data)
  {
    ...
    if ($transaction_data['replaced_by'] != '') $replaced_class = 'replaced ';
    $display = '
      <tbody id="'.$transaction_data['transaction_id'].'">
        <tr class="'.$replaced_class.'">
        ...
        <tr class="'.$replaced_class.'">
          ...
          <td>'.$transaction_data['delcode_id'].'</td>
...
function get_delcode_list ()
  {
    global $connection;
    // Build delivery code list
    $query = '
      SELECT
        delcode_id,
        delcode
...
<html>
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="/adjust_ledger.css">
    <script src="/ajax/jquery.js" type="text/javascript"></script>
    <script src="/ajax/jquery-ui.js" type="text/javascript"></script>
    <script src="/adjust_ledger.js" type="text/javascript"></script>
  </head>
  <body lang=""en-us">';
  }
