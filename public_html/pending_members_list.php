...
if ($_POST['action'] == 'Submit')
  {
    ...
    $count = 0;
    // Iterate through all the members on the pending/unpaid members page
    while ( $count++ <= $counter )
      {
                  ...
                  INSERT INTO
                    ...
                    VALUES
                      (
                        ...
                        "'.mysql_real_escape_string (round ($payment_amount * -1, 2)).'",
...
$sql = @mysql_query($query, $connection) or die("Couldn't execute query 7.<br />".mysql_error());
...
while ( $row = mysql_fetch_array($sql) )
  {
    ...
    $query2 = '
      SELECT
        MAX('.TABLE_ORDER_CYCLES.'.delivery_date) AS recent_delivery_date
      FROM
        '.TABLE_ORDER_CYCLES.'
      JOIN
        '.TABLE_BASKET_ALL.' ON '.TABLE_ORDER_CYCLES.'.delivery_id = '.TABLE_BASKET_ALL.'.delivery_id
      WHERE
    ...
    if ($membership_dues != 0 || $pending == 1)
      {
        ...
        //not yet approved
            ...
            $display .= '
              <tr bgcolor = "'.$bg.'">
                <td>
                  <strong>Member #'.$member_id.'</strong> ['.$username.']<br /><br />
  ...
  <tr>
    <td align="left">
      <div align="center" style="margin-bottom:8px;">
        <h3>'.$num.' Pending or Unpaid Members</h3><h5>Listed Newest First</h5>
...
$page_subtitle_html = '<span class="subtitle">Pending and Unpaid Members</span>';
$page_title = 'Membership Information: Pending and Unpaid Members';
