...
while ( $row = mysql_fetch_array($result) )
  {
    ...
    $row['row_activity_link'] = row_activity_link_calc($row, $pager);
    ...
    if ($row[$major_division] != $$major_division_prior && $show_major_division == true)
      {
        if ($listing_is_open)
          {
            if ($show_minor_division) $display .= minor_division_close($row);
    ...
    if ($row[$minor_division] != $$minor_division_prior && $show_minor_division == true)
      {
        if ($listing_is_open)
          {
            $display .= minor_division_close($row);
            $listing_is_open = 0;
          }
...
//alert(data);
    ...
    if (new_quantity > 0) // The item is in the basket
      {
    ...
    else // The item is not in the basket
      {
        document.getElementById("add"+product_id).style.display = "none";
    ...
    if (checked_out == 1) {
      document.getElementById("checkout"+product_id).innerHTML = "<input type=\"image\"class=\"checkout_check\" src=\"'.PATH.'/grfx/checkout-ccs.png\" onclick=\"AddToCart("+product_id+","+product_version+",\'no_checkout\'); return false;\"><span class=\"checkout_text\">Ordered!</span>";
