            ...
            while ($row = mysql_fetch_array($sql))
              {
                ...
                if ($level <= $_GET['depth'])
                  { //Prepare output formatting
                    ...
                    //$list_markup .= '<li class="cat'.$level.'"><a href="'.$_SERVER['PHP_SELF'].'?category_id='.$category_id.'">'.$category_name."</a>$item_count_markup $item_new_count_markup</li>\n";
...
if ($base_category == 0)
...
else
  {
    ...
    $overall_category_name = '<h3 class="inline"><a href="'.$_SERVER['PHP_SELF'].'?category_id='.$overall_parent_id.'&offset='.($_GET['offset'] - 1).'"><strong>&laquo;'.$row['category_name'].'</strong></a></h3>';
