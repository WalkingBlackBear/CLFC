...
if ($_POST["addCycle"] == "doIT") {
	...
	// check to see if the cycle we're trying to add already exists to prevent duplicates.  Match on closing date.
	$cs = "SELECT `delivery_id` FROM `order_cycles` WHERE `date_closed`='$dateClosed' OR `date_open`='$dateOpen' OR `order_fill_deadline`='$fillDeadline' OR `delivery_date`='$delDate'";
	$cq = mysql_query($cs,$connection);
	$cycleExists = mysql_num_rows($cq); // if 0/false, we're good to go.  Otherwise, one of those dates already exists in the DB
	if ($cycleExists) {
		$display_admin .= "<h1>ERROR: An order cycle already exists with one of the new dates. Check with Chris!</h1>";
	} else {
		// order cycle does not exist.  Go ahead and add it!
	...
	$nextOpenDate = (date('N', time()) == '6' ? strtotime("today 2pm") : strtotime("Saturday 2pm"));
