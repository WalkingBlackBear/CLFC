...
function geninvoice ($member_id, $basket_id, $delivery_id, $use)
  {
    // Just in case the basket_id was not passed, set it...
    if (! isset ($basket_id))
      {
        $basket_id = CurrentBasket::basket_id ();
      }

    // Since this may be used to pull past invoices, we need to change the active_delivery_id
    ActiveCycle::set_active_delivery_id ($delivery_id);
    
    global $font, $fontface, $connection;
    // These globals are used for the tax_rates function in tax_functions.php
    global $show_name, $preferred_name,
           // For delivery to home address:
...
// // TEST CODE TO EVAL CUSTOMER_INVOICE_TEMPLATE FROM DATABASE
//     $query = '
//       SELECT
...
//       {
//         eval ($row['customer_invoice_template']);
//       }
// // END TEST CODE TO EVAL CUSTOMER_INVOICE_TEMPLATE FROM DATABASE
...
////////////////////////////////////////////////////////////////////////////////
    ...
    while ( $row = mysql_fetch_array($result) )
      {
        ...
        $authorization = explode (',', $auth_type);
      }
    // Initialize variables for product-listing section
    $producer_id_prior = '';
    $product_display_output = '';
    $unfilled_random_weight = 0;
    // Need to do this once and set these values for cases where there were no
    // products ordered and just adjustments made
    if (SHOW_ACTUAL_PRICE)
      {
        if (in_array ('institution', $authorization))
          {
            $price_multiplier = 1 + ActiveCycle::wholesale_markup ();
            $adjustment_markup = 1 + ActiveCycle::wholesale_markup ();
            $coop_markup = 0;
          }
        else
          {
            $price_multiplier = 1 + ActiveCycle::retail_markup ();
            $adjustment_markup = 1 + ActiveCycle::retail_markup ();
            $coop_markup = 0;
          }
      }
    else // if (not SHOW_ACTUAL_PRICE)
      {
        if (in_array ('institution', $authorization))
          {
            $price_multiplier = 1;
            $coop_markup = ActiveCycle::wholesale_markup ();
            $adjustment_markup = 1;
          }
        else
          {
            $price_multiplier = 1;
            $coop_markup = ActiveCycle::retail_markup ();
            $adjustment_markup = 1;
          }
      }
    ...
    $sql2 = '
      ...
      LEFT JOIN
        '.TABLE_SUBCATEGORY.' ON '.TABLE_SUBCATEGORY.'.subcategory_id = '.TABLE_BASKET.'.subcategory_id
      LEFT JOIN
        '.TABLE_CATEGORY.' ON '.TABLE_CATEGORY.'.category_id = '.TABLE_SUBCATEGORY.'.category_id
      LEFT JOIN
        '.TABLE_PRODUCT_STORAGE_TYPES.' ON '.TABLE_PRODUCT_STORAGE_TYPES.'.storage_id = '.TABLE_BASKET.'.storage_id
      WHERE
    ...
    while ( $row = mysql_fetch_array($result2) )
      {
        ...
        if (SHOW_ACTUAL_PRICE)
          {
            if (in_array ('institution', $authorization))
              {
                $price_multiplier = (1 + ActiveCycle::wholesale_markup ()) * (1 + $product_adjust_fee + $subcat_adjust_fee + $producer_adjust_fee);
                $adjustment_markup = 1 + ActiveCycle::wholesale_markup ();
                $coop_markup = 0;
              }
            else
              {
                $price_multiplier = (1 + ActiveCycle::retail_markup ()) * (1 + $product_adjust_fee + $subcat_adjust_fee + $producer_adjust_fee);
                $adjustment_markup = 1 + ActiveCycle::retail_markup ();
                $coop_markup = 0;
              }
          }
        else // if (not SHOW_ACTUAL_PRICE)
          {
            if (in_array ('institution', $authorization))
              {
                $price_multiplier = 1 + $product_adjust_fee + $subcat_adjust_fee + $producer_adjust_fee;
                $coop_markup = ActiveCycle::wholesale_markup ();
                $adjustment_markup = 1;
              }
            else
        ...
        // Set up a value for display of coop_markup percentage
        $coop_markup_display = $coop_markup * 100;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    $query = '
      ...
      WHERE
        transaction_basket_id = "'.mysql_real_escape_string ($basket_id).'"
        ...
        AND NOT
          (
            ...
            AND transaction_type = 64 /* Only one ttype_id=64 is allowed for any given delivery_id */
          )
      GROUP BY
        transaction_id
      ORDER BY
    ...
    while ( $row = mysql_fetch_array($result) )
      {
        ...
        if ( $transaction_taxed == 1 && $mem_taxexempt != 1)
          {
            if (SHOW_ACTUAL_PRICE) $taxed_adjustment_cost = $taxed_adjustment_cost + round ($transaction_amount * $adjustment_markup, 2);
            else $taxed_adjustment_cost = $taxed_adjustment_cost + round ($transaction_amount, 2);
            $taxable_product = $taxable_product_flag;
          }
        else
          {
    ...
    // Get ALL membership activity (transactions with ttype_parent=40) EXCEPT any per-order fees for the current order
    // but DO NOT include any fees that happened AFTER this order closed.
    $query = '
      SELECT
        '.TABLE_TRANSACTIONS.'.*,
      ...
      FROM
        '.TABLE_TRANSACTIONS.'
      LEFT JOIN '.TABLE_TRANS_TYPES.' ON '.TABLE_TRANS_TYPES.'.ttype_id = '.TABLE_TRANSACTIONS.'.transaction_type
      LEFT JOIN '.TABLE_ORDER_CYCLES.' ON '.TABLE_TRANSACTIONS.'.transaction_delivery_id = '.TABLE_ORDER_CYCLES.'.delivery_id
      WHERE
        transaction_member_id = "'.mysql_real_escape_string ($member_id).'"
        AND '.TABLE_TRANS_TYPES.'.ttype_parent = "40"'.
//         AND NOT
//           (
//             /* do not show per-order fee for current order */
//             /* Only a single ttype_id=64 is allowed for any given delivery_id */
//             transaction_delivery_id = "'.mysql_real_escape_string ($delivery_id).'"
//             AND transaction_type = 64
//           )
$SOMETHING_BOGUS_TO_END_THE_COMMENT.'
...
////////////////////////////////////////////////////////////////////////////////
    ...
    $query = '
      SELECT
        ...
        '.TABLE_BASKET_ALL.'.order_cost,
        '.TABLE_BASKET_ALL.'.order_cost_type,
      ...
      WHERE
        ...
        AND '.TABLE_ORDER_CYCLES.'.delivery_id = "'.$delivery_id.'"
      ';
    $result = @mysql_query($query, $connection) or die(mysql_error());
    while ( $row = mysql_fetch_array($result) )
      {
    ...
    list ($city_name, $copo_city, $city_tax_rate, $county_name, $copo_county, $county_tax_rate, $state_id, $state_tax_rate) = tax_rates ($delivery_id);
// WAS:    list ($city_name, $copo_city, $city_tax_rate, $county_name, $copo_county, $county_tax_rate, $state_id, $state_tax_rate) = tax_rates ();
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if ($order_cost_type == 'fixed' && $order_cost != 0 )
      {
        $order_cost_description = '$&nbsp;'.number_format ($order_cost, 2);
      }
    elseif ($order_cost_type == 'percent')
      {
        $order_cost_description = number_format ($order_cost, 2).'&nbsp;%';
        $order_cost = round (($coop_fee_basis * $order_cost / 100), 2);
      }
    ...
    // while the $paypal_fee is the actual amount we set aside for paypal, which is nothing if not paying by paypal.
    $paypal_fee = 0;
    $potential_paypal_fee = round (((($total_basket_cost + $exempt_adjustment_cost + $delcharge + $coop_fee + 0.30) / 0.971) * 0.029) + 0.30, 2);
// WAS:    $potential_paypal_fee = round (((($subtotal_1 + $exempt_adjustment_cost + 0.30) / 0.971) * 0.029) + 0.30, 2);
    ...
    $subtotal_2 = $total_current_cost + $total_tax + $delcharge + $coop_fee + $exempt_adjustment_cost + $potential_paypal_fee;
// WAS:    $subtotal_2 = $total_current_cost + $total_tax + $delcharge + $coop_fee + $potential_paypal_fee;
    ...
    if ( $use == 'adminfinalize' )
      {
        ...
        $sqlo = '
          UPDATE
            '.TABLE_BASKET_ALL.'
          SET
            subtotal = "'.mysql_real_escape_string ($total_basket_cost).'", /* WAS $total_current_cost */
            sh = "'.mysql_real_escape_string ($coop_fee).'",
            transcharge = "0",
            delivery_cost = "'.mysql_real_escape_string ($delcharge).'",
            invoice_content = "'.mysql_real_escape_string ($overall_invoice_display_output).'",
            grand_total = "'.mysql_real_escape_string ($grand_total).'", /* did include $order_cost */
            grand_total_coop = "'.mysql_real_escape_string ($grand_total_coop + $order_cost).'",
            surcharge_for_paypal = "'.mysql_real_escape_string ($paypal_fee).'",
            finalized = "1"
          WHERE
        ...
        if ( $number_of_rows != 0 )
          {
            $sqltx = '
              ...
              SET
                delivery_id = "'.mysql_real_escape_string ($delivery_id).'",
                taxable_total = "'.mysql_real_escape_string ($total_taxable_cost).'",
                exempt_total = "'.mysql_real_escape_string ($total_exempt_cost).'",
                taxrate_state = "'.mysql_real_escape_string ($state_tax_rate).'",
                collected_statetax = "'.mysql_real_escape_string ($state_sales_tax).'",
                copo_city = "'.mysql_real_escape_string ($copo_city).'",
                taxrate_city = "'.mysql_real_escape_string ($city_tax_rate).'",
                collected_citytax = "'.mysql_real_escape_string ($city_sales_tax).'",
                copo_county = "'.mysql_real_escape_string ($copo_county).'",
                taxrate_county = "'.mysql_real_escape_string ($county_tax_rate).'",
                collected_countytax = "'.mysql_real_escape_string ($county_sales_tax).'",
                last_modified = now()
              WHERE
                basket_id = "'.mysql_real_escape_string ($basket_id).'"';
            ...
            $sqlinsert = '
              ...
              VALUES
                (
                  ...
                  "'.mysql_real_escape_string ($total_taxable_cost).'",
                  "'.mysql_real_escape_string ($total_exempt_cost).'",
                  "'.mysql_real_escape_string ($state_tax_rate).'",
                  "'.mysql_real_escape_string ($state_sales_tax).'",
                  "'.mysql_real_escape_string ($copo_city).'",
                  "'.mysql_real_escape_string ($city_tax_rate).'",
                  "'.mysql_real_escape_string ($city_sales_tax).'",
                  "'.mysql_real_escape_string ($copo_county).'",
                  "'.mysql_real_escape_string ($county_tax_rate).'",
                  "'.mysql_real_escape_string ($county_sales_tax).'")';
        ...
        $query = '
          ...
          VALUES
            (
              ...
              "'.mysql_real_escape_string ($total_basket_cost).'", /* WAS: $total_basket_cost, then $total_current_cost, and now back to $total_basket_cost */
        ...
        $query = '
          ...
          VALUES
            (
              "29",
              "'.mysql_real_escape_string ($row['ttype_name']).'",
              "'.mysql_real_escape_string ($total_tax).'",
              "'.mysql_real_escape_string ($_SESSION['member_id']).'",
              "'.mysql_real_escape_string ($member_id).'",
              "'.mysql_real_escape_string ($basket_id).'",
              "'.mysql_real_escape_string ($delivery_id).'",
        ...
        $query = '
          ...
          VALUES
            (
              "30",
              "'.mysql_real_escape_string ($row['ttype_name']).'",
              "'.mysql_real_escape_string ($coop_fee).'",
              "'.mysql_real_escape_string ($_SESSION['member_id']).'",
              "'.mysql_real_escape_string ($member_id).'",
              "'.mysql_real_escape_string ($basket_id).'",
              "'.mysql_real_escape_string ($delivery_id).'",
        ...
        $query = '
          ...
          VALUES
            (
              "32",
              "'.mysql_real_escape_string ($row['ttype_name']).'",
              "'.mysql_real_escape_string ($paypal_fee).'",
              "'.mysql_real_escape_string ($_SESSION['member_id']).'",
              "'.mysql_real_escape_string ($member_id).'",
              "'.mysql_real_escape_string ($basket_id).'",
              "'.mysql_real_escape_string ($delivery_id).'",
        ...
        $query = '
          ...
          VALUES
            (
              "34",
              "'.mysql_real_escape_string ($row['ttype_name']).'",
              "'.mysql_real_escape_string ($pay_this_amount - $previous_balance).'", /* WAS $subtotal_2 */
              "'.mysql_real_escape_string ($_SESSION['member_id']).'",
              "'.mysql_real_escape_string ($member_id).'",
              "'.mysql_real_escape_string ($basket_id).'",
              "'.mysql_real_escape_string ($delivery_id).'",
            ...
            if (mysql_num_rows ($result) == 0)
              {
                ...
                $query = '
                  ...
                  SET
                    ...
                    transaction_amount = '.mysql_real_escape_string ($order_cost).',
                    transaction_user = "'.mysql_real_escape_string ($_SESSION['member_id']).'",
            ...
            else
              {
                ...
                $query = '
                  ...
                  WHERE
                    ...
                    AND transaction_basket_id = "'.mysql_real_escape_string ($basket_id).'"
