...
if ($check_validation == true && $_POST['submit_action'] != 'Cancel')
  {
    ...
    $_POST['listing_auth_type']      = preg_replace("/[^member|^institution|^unlisted|^archive|^unfi]/",'',$_POST['listing_auth_type']);
    ...
    if ( ! $_POST['unit_price'] )
      {
        array_push ($error_array, 'Please enter a unit price.');
        $warn['pricing'] = ' warn';
        $okay_to_post = false;
      }
    if ( ! $_POST['pricing_unit'] )
      {
        array_push ($error_array, 'Please enter a pricing unit.');
        $warn['pricing'] = ' warn';
        $okay_to_post = false;
      }
    if ( ! $_POST['storage_id'] )
    ...
    if ( $_POST['random_weight'] && ( ! $_POST['minimum_weight'] || ! $_POST['maximum_weight']) )
      {
        array_push ($error_array, 'You have selected Yes for random weight product. If this is a random weight product you need to enter an approximate minimum and maximum weight.');
        $warn['random_weight'] = ' warn';
        $warn['min_max_weight'] = ' warn';
        $okay_to_post = false;
      }
            ...
            if ($_POST[$field] != $db_product_info[$field])
              {
                ...
                if (in_array ($field, unserialize (FIELDS_REQ_CONFIRM))) $force_confirmation = true;
              }
          }
        // Reserve a new product in the database, then all we need to do is update it later
        $new_product_row = get_next_product_id ($producer_id);
