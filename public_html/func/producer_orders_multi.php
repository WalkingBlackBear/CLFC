...
$sqlp = '
  ...
  FROM
    '.TABLE_PRODUCER.'
  WHERE
    '.TABLE_PRODUCER.'.producer_id = "'.mysql_real_escape_string ($_SESSION['producer_id_you']).'"
  GROUP BY
...
$sort_array = array (
  ...
  'Member Name' => array ('var_name' => 'preferred_name', 'db_sort' => ''.TABLE_MEMBER.'.last_name ASC, '.TABLE_MEMBER.'.first_name ASC, '.TABLE_MEMBER.'.business_name ASC'),
  ...
  'Product Name' => array ('var_name' => 'product_name', 'db_sort' => ''.TABLE_BASKET.'.product_name ASC'),
...
$sqlpr = '
  ...
  LEFT JOIN '.TABLE_PRODUCT_STORAGE_TYPES.'
    ON '.TABLE_PRODUCT_STORAGE_TYPES.'.storage_id = '.TABLE_BASKET.'.storage_id
  LEFT JOIN '.TABLE_SUBCATEGORY.'
    ON '.TABLE_BASKET.'.subcategory_id = '.TABLE_SUBCATEGORY.'.subcategory_id
  ...
  LEFT JOIN '.TABLE_ROUTE.'
    ON '.TABLE_DELCODE.'.route_id = '.TABLE_ROUTE.'.route_id
  LEFT JOIN '.TABLE_HUBS.'
    ON '.TABLE_HUBS.'.hub_id = '.TABLE_DELCODE.'.hub
  WHERE
    '.TABLE_BASKET.'.producer_id = "'.mysql_real_escape_string ($producer_id).'"
...
while ( $row = mysql_fetch_array($resultpr) )
  {
            ...
            if ( $show_form )
              {
                ...
                if ( $_POST['Update'] == 'Update All' && $_POST['total_weight'.$update_id] != $total_weight )
                  {
                    ...
                    $query_update = '
                      ...
                      WHERE
                        basket_id = "'.mysql_real_escape_string ($c_basket_id).'"
                ...
                $display_weight = '<input type="text" name="total_weight'.$update_id.'" value="'.$total_weight.'" size="5" maxlength="11"><br>'.Inflect::pluralize ($pricing_unit).$total_weight_updated;
              }
            else
              {
    ...
    $header_member = '
      <b>Member:</b> '.$preferred_name.' (#'.$member_id.')
      <br>'.$home_phone.' <a href="mailto:'.$email_address.'">'.$email_address.'</a>
      <br>'.(convert_route_code($basket_id, $member_id, $last_name, $first_name, $business_name, $a_business_name, $hub, $delcode_id, $deltype, $truck_code, $storage_code, $preferred_name, $product_name, $quantity, $ordering_unit, $product_id, $item_price, $delcode));
    $header_member_name = '
      <b>Member:</b> '.$preferred_name.' (#'.$member_id.')
      <br>'.$home_phone.' <a href="mailto:'.$email_address.'">'.$email_address.'</a>
      <br>'.(convert_route_code($basket_id, $member_id, $last_name, $first_name, $business_name, $a_business_name, $hub, $delcode_id, $deltype, $truck_code, $storage_code, $preferred_name, $product_name, $quantity, $ordering_unit, $product_id, $item_price, $delcode));
    ...
    if ( $notes )
      {
        ...
        $line_item_member .= '<br><b>Customer note: </b>'.$notes;
      }
    ...
    elseif ( $sort_array[$sort1]['var_name'] == 'preferred_name' )
      {
    ...
    elseif ( $sort_array[$sort2]['var_name'] == 'preferred_name' )
      {
    ...
    elseif ( $sort_array[$sort3]['var_name'] == 'preferred_name' )
      {
    ...
    $producer_orders_multi .= $line_html;
  }
$producer_orders_multi .= '
  <tr>
