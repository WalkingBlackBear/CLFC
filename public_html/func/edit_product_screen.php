...
function format_help_link ($target)
  {
    ...
    if (in_array ($target, unserialize (FIELDS_REQ_CONFIRM)))
      {
...
$selected = ($product_info['inventory_id'] == 0 ? ' selected' : '');
$inventory_select = '
    <select name="inventory_id" onChange="if (this.value==0) document.getElementById(\'inventory_pull\').style.display=\'none\'; else document.getElementById(\'inventory_pull\').style.display=\'\';">
      <optgroup label="SELECT A FUNCTION:">
        <option value="0"'.$selected.'>DO NOT USE ANY INVENTORY</option>
        <option value="-1">CREATE A NEW INVENTORY UNIT</option>
      </optgroup>
...
if     ($product_info['listing_auth_type'] == 'archive')     $listing_auth_type_chk3 = ' checked';
...
if( $action == 'edit' )
  {
    $display = '<form action="'.$PHP_SELF.'?product_id='.$product_id.'&product_version='.$product_version.'&producer_id='.$producer_id.'&a='.$_GET['a'].'" method="post">';
  }
elseif ( $action == 'add' )
  {
    $display = '<form action="'.$PHP_SELF.'?producer_id='.$producer_id.'&a='.$_GET['a'].'" method="post">';
  }
$display .= '
...
$display .= '
    ...
    <tr'.$norm_bg.'>
      ...
      <input type="radio" name="listing_auth_type" value="unlisted"'.$listing_auth_type_chk2.'> Unlisted<br>
        <input type="radio" name="listing_auth_type" value="archive"'.$listing_auth_type_chk3.'> Archived<br>
        </b>
      </td>
    </tr>
    <tr'.$norm_bg.'>
    ...
    <tr'.$norm_bg.'>
      ...
      <td>Each item purchased will pull <input type="text" name="inventory_pull" value="'.$product_info['inventory_pull'].'" size=3 maxlength="6"> unit(s) of...<br>'.$inventory_select.'<span id="inventory_pull"> from inventory.</span>
        </div>
      </td>
    </tr>
    <tr'.$norm_bg.'>
...
$display .= '
    ...
    <tr'.$norm_bg.'>
      ...
      <td>
       $&nbsp;<input type="text" name="extra_charge" value="'.number_format((double)$product_info['extra_charge'], 2).'" size="6" maxlength="6"><br>
    ...
    <tr '.$ranwt_bg.' id="max_min"'.$chk3d.'>
      ...
      <td>
        ...
        <input type="text" name="minimum_weight" value="'.$product_info['minimum_weight'].'" size="6" maxlength="6">
        &nbsp;&nbsp;&nbsp;&nbsp;<b>Approx. Maximum weight</b>:
        <input type="text" name="maximum_weight" value="'.$product_info['maximum_weight'].'" size="6" maxlength="6"><br>
    ...
    $display .= '
      <tr '.$cntrl_bg.'>
        ...
        <td align="left">
          ...
          <input type="checkbox" name="save_as_new" value="Save as a New Product"> Check here to keep the original product as it was and save these changes as a new product. The new product will have the same image, so it may need to be changed.
        </td>
      </tr>
      <tr '.$cntrl_bg.'>
