...
elseif (isset ($_GET['sort']) && $_GET['sort'] == 'delcode_id') $sort = 'delcode_id';
else
  {
    $sort = 'member_name';
    $switch = '&amp;order=desc';
...
elseif ($sort == 'delcode_id') $order_by = 'ORDER BY '.TABLE_DELCODE.'.delcode_id'.$order;
...
while ( $row = mysql_fetch_array($result) )
  {
        ...
        SUM(
          IF(random_weight = 1,
            IF(pricing_unit = ordering_unit,'.
              ...
              6lb  of a random weight (5lb - 7 lb) item does NOT get multiplied by the qty=6 ordered */
              '
              (unit_price * ((maximum_weight + minimum_weight) / 2)),'.
            ...
            '
            (unit_price * quantity))
          * ((customer_fee_percent + 100) / 100)) AS estimate_total,
    ...
    $display .= '
      ...
      <td>'.$delcode_id.'</td>
      <td># '.$member_id.'</td>
      <td><a href="product_list.php?type=basket&amp;delivery_id='.$delivery_id.'&amp;member_id='.$member_id.'&amp;basket_id='.$basket_id.'"><strong>'.$last_name.':</strong> '.$preferred_name.'</a></td>
      <td>';
    $sqlp = '
      SELECT
    ...
    $display .= '</td>';
    $display .= '<td valign="top"><font size="2"><a href="show_report.php?type=customer_invoice&amp;delivery_id='.$delivery_id.'&amp;member_id='.$member_id.'">View Invoice</a></font></td>';
    $display .= '</tr>';
...
<table id="customer_baskets">
  <tr>
    ...
    <th><a href="'.$_SERVER['SELF'].'?delivery_id='.$delivery_id.'&sort=delcode_id'.($sort == 'delcode_id' ? $switch : '&amp;order=asc').'">Del. Code '.($sort == 'delcode_id' ? $order_arrow : '').'</a></th>
    <th><a href="'.$_SERVER['SELF'].'?delivery_id='.$delivery_id.'&sort=member_id'.($sort == 'member_id' ? $switch : '&amp;order=asc').'">Mem. ID '.($sort == 'member_id' ? $order_arrow : '').'</a></th>
    <th><a href="'.$_SERVER['SELF'].'?delivery_id='.$delivery_id.'&sort=member_name'.($sort == 'member_name' ? $switch : '&amp;order=asc').'">Name '.($sort == 'member_name' ? $order_arrow : '').'</a> (Click below to view basket)</th>
...
$page_specific_css = '
  <style type="text/css">
    ...
    #customer_baskets td:nth-child(3) /* delcode_id */
      {
        text-align:center;
      }
