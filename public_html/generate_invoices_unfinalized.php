...
$query = '
  ...
  LEFT JOIN
    '.TABLE_MEMBER.' ON '.TABLE_BASKET_ALL.'.member_id = '.TABLE_MEMBER.'.member_id
  WHERE
    ...
    AND '.TABLE_BASKET_ALL.'.finalized = 0
  GROUP BY
    '.TABLE_BASKET_ALL.'.member_id
  ORDER BY
...
while($row = mysql_fetch_array($result))
  {
    ...
    $sql = '
      ...
      WHERE
        ...
        AND '.TABLE_BASKET.'.random_weight = "1"
    ...
    $content .= '          <li id="basket_id:'.$basket_id.'" class="c_complete"><a href="orders_invoice.php?delivery_id='.$delivery_id.'&basket_id='.$basket_id.'&member_id='.$member_id.' " target="_blank"><div class="c_list_cid">'.$ready_begin.$row['member_id'].$ready_end.'</div><div class="c_list_name">'.$hub.': '.$delcode_id.' ['.$last_name.', '.$first_name.']</div></a></li>';
  }
...
while($row = mysql_fetch_array($resultp))
  {
    ...
    $content .= '          <li id="producer_id:'.$producer_id.'" class="p_complete"><a href="orders_prdcr_cust_storage.php?producer_id='.$producer_id.'&delivery_id='.$delivery_id.'&display_only=true" target="_blank"><div class="p_list_pid">'.$ready_begin.$producer_id.$ready_end.'</div><div class="p_list_name">'.$business_name.'</div></a></li>';
