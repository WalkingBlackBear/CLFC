    ...
    while ( $row = mysql_fetch_array($sql) )
      {
        ...
        $sql2 = mysql_query('
          ...
          FROM
            '.TABLE_TRANS_TYPES.'
          WHERE
            ttype_parent="'.mysql_real_escape_string ($row["ttype_id"]).'"
          ORDER BY
            ttype_name ASC');
...
<table style="border:1px solid;font-family:Verdana;font-size:10pt;">
  <tr style="background-color:#EEEEEE;">
    ...
    <td align="center">Member ID:
      <input type="text" name="search_criteria[transaction_member_id]" value="'.$_POST['search_criteria']['transaction_member_id'].'" size="4" maxlength="8">Delivery ID:
      <input type="text" name="search_criteria[transaction_delivery_id]" value="'.$_POST['search_criteria']['transaction_delivery_id'].'" size="3" maxlength="4">Batch No.:
      <input type="text" name="search_criteria[transaction_batchno]" value="'.$_POST['search_criteria']['transaction_batchno'].'" size="3" maxlength="8">';
...
if ( $_POST['search_criteria']['day2'] )
  {
    $content_report .= '
      <option value="'.$_POST['search_criteria']['day2'].'">'.$_POST['search_criteria']['day2'].'</option>';
  }
else
  {
    $content_report .= '
...
if ( $_POST['search_criteria']['year2'] )
  {
    $content_report .= '
      <option value="'.$_POST['search_criteria']['year2'].'">'.$_POST['search_criteria']['year2'].'</option>';
  }
else
  {
    $content_report .= '
...
$page_subtitle_html = '<span class="subtitle">Financial Report</span>';
$page_title = 'Reports: Financial Report';
