    ...
    if ($this_page == $page)
      {
        $page_data .= '
          <div id="member_id'.$row['member_id'].'" class="basket_section">
            ...
            <span class="delcode_id">[N/A]</span>
    ...
    if ($member_count == PER_PAGE)
      {
        ...
        $pager_display .= '
          <a href="'.$_SERVER['PHP_SELF'].'?page='.$this_page.'" class="'.($this_page == $page ? 'current' : '').($this_page == 1 ? ' first' : '').'">&nbsp;'.$page_starting_member.'-'.$page_ending_member.'&nbsp;</a>';
        $this_page ++;
        $member_count = 0;
      }
...
if ($member_count != PER_PAGE)
  {
    // Get the ending member_id for this page
    $pager_display .= '
      <a href="'.$_SERVER['PHP_SELF'].'?page='.$this_page.'" class="'.($this_page == $page ? 'current' : '').' last">&nbsp;'.$page_starting_member.'-'.$page_ending_member.'&nbsp;</a>';
  }
$pager_display .= '
  </div>';
...
//         <span class="delcode_id">['.$delcode_id.']</span>
