...
$checkbox = '<img src="/shop/grfx/checkbox.gif" style="height:1em;vertical-align:text-top;">';
...
$route_code = convert_route_code((array) $row);
        ...
        if ($row->member_id != $member_id_prior || $row->delcode != $delcode_prior) $output .=
          "\n\n ".str_pad('('.$row->member_id.')', 6, ' ', STR_PAD_LEFT).'  '.$route_code.' - '.$row->first_name.' '.$row->last_name.
          "\n  ".str_pad($row->product_id, 7, ' ', STR_PAD_LEFT).'   '.$checkbox.'  '.str_pad($row->storage_code, 6).str_pad(substr($row->product_name, 0, 40), 40).'     '.'('.$row->quantity.') - '.Inflect::pluralize_if ($row->quantity, $row->ordering_unit);
//         elseif ($row->storage_code != $storage_code_prior) $output .=
//           "\n      ".str_pad('('.$row->member_id.')', 6, ' ', STR_PAD_LEFT).str_pad('['.$row->product_id.']', 10, ' ', STR_PAD_LEFT).'  $checkbox  '.str_pad(substr($row->product_name, 0, 40), 40).'     '.'('.$row->quantity.') - '.Inflect::pluralize_if ($row->quantity, $row->ordering_unit);
...
if ($_GET['output'] == 'pdf')
  {
    $fp = fopen( FILE_PATH.PATH.'members/temp/route_list.html', a);
    fwrite($fp, $output);
  }
else
  {
    include("template_header.php");
    echo '<a href="'.$_SERVER['PHP_SELF'].'?type='.$type.'&delivery_id='.$delivery_id.($_GET['producer_id'] ? '&producer_id='.$_GET['producer_id'] : '').'&output=pdf">Download as PDF</a>';
    if (! $_GET['producer_id'])
      {
        echo '
          <style type="text/css">
...
if ($_GET['output'] == 'pdf')
  {
    ...
    flush();
    passthru('htmldoc -t pdf --webpage '.FILE_PATH.PATH.'members/temp/route_list.html');
    unlink(FILE_PATH.PATH.'members/temp/route_list.html');
  }
