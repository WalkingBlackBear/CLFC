...
function total_display_calc($data)
  {
    ...
    if ($data['random_weight'] && $data['total_weight'] == 0) $total_display = '
      $[pending]';
    elseif ($data['unit_price']  != 0) $total_display .= '
      <span id="customer_adjusted_cost'.$data['bpid'].'">$&nbsp;'.number_format($data['customer_adjusted_cost'], 2).'</span>';
    // If there is content so far, then add a newline
    if ($total_display) $total_display .= '
      <br>';
...
// Original retail pricing (commented out) is not used ..............................
...
//     '<span class="whsle">'.($_GET['type'] == 'producer_list' ? 'Whsle: ' : '').'$'.number_format(round($data['display_unit_wholesale_price'], 2), 2).'/'.Inflect::singularize ($data['pricing_unit']).'</span><br>'
//     : '').
//     ($data['extra_charge'] != 0 ?
...
//     : '');
//   };
// ........................... but instead we use base pricing for the pricing display
function pricing_display_calc($data)
  { return
    ($data['unit_price'] != 0 ?
...
function image_display_calc($data)
  { return
    ($data['image_id'] ?
    '<img src="'.PATH.'getimage.php?image_id='.$data['image_id'].'" width="100" name="img'.$data['image_id'].'" onclick="javascript:img'.$data['image_id'].'.width=300" onMouseOut="javascript:img'.$data['image_id'].'.width=100" hspace="5" border="1" align="left" alt="Click to enlarge '.htmlentities ($data['product_name'], ENT_QUOTES).'">'
    : '');
  };
...
function business_name_display_calc($data)
  { return
    '<a href="product_list.php?type=producer_id&producer_id='.$data['producer_id'].'">'.$data['producer_name'].'</a>';
  };
      ...
      ($data['order_open'] ?
        ($data['availability'] == true ?
          ($data['basket_quantity'] > 0 || !$data['inventory_id'] || $data['inventory_quantity'] > 0 ?
            ...
            '<form action="'.$_SERVER['PHP_SELF'].'?type='.$_GET['type'].'#X'.$data['product_id'].'" method="post">
               <input id="add'.$data['product_id'].'" class="basket_add" type="image" name="basket_add" src="grfx/basket_add.png" width="24" height="24" border="0" alt="Submit" onclick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'add\'); return false;" '.($data['basket_quantity'] > 0 ? ($data['inventory_id'] && $data['inventory_quantity'] == 0 ? 'style="display:none;"' : '') : 'style="display:none;"').'>
               <input id="sub'.$data['product_id'].'" class="basket_sub" type="image" name="basket_sub" src="grfx/basket_sub.png" width="24" height="24" border="0" alt="Submit" onclick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'sub\'); return false;" '.($data['basket_quantity'] > 0 ? '' : 'style="display:none;"').'>
               ...
               <input id="basket_empty'.$data['product_id'].'" class="basket" type="image" name="basket" src="grfx/basket-egi_add.png" width="48" height="48" border="0" alt="Submit" onClick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'add\'); return false;" '.($data['basket_quantity'] > 0 ? 'style="display:none;"' : '').'>
               <img id="basket_full'.$data['product_id'].'" class="basket" src="grfx/basket-fcs.png" width="48" height="48" border="0" '.($data['basket_quantity'] > 0 ? '' : 'style="display:none;"').'>
               </div>
             </form>
...
($_SESSION['member_id'] == 1 ? '
        ...
        ($data['delcode_id'] && $data['delivery_postal_code'] ?
          ($data['basket_checked_out'] != 0 ? '
            <img class="checkout_check" src="grfx/checkout-ccs.png">
            <span class="checkout_text">Checked</span>'
            : '
            <input type="image" class="checkout_check" src="grfx/checkout-g.png" onclick="AddToCart(0,0,\'checkout_basket\'); return false;">
    ...
    switch ($minor_division)
      {
        ...
        case 'producer_name':
          $header = '
            <tr>
              <td colspan="6" class="header_minor">
                <a href="'.BASE_URL.PATH.'producers/'.$data['producer_link'].'">'.$data['producer_name'].'</a>
              </td>
            </tr>';
          break;
        // Majory division on producer
      ...
      <td>
        ...
        ($data['delcode_id'] && $data['delivery_postal_code'] ?
          ($data['checked_out'] ? '
            ...
            : '
            <input type="image" class="checkout_check" src="grfx/checkout-g.png" onclick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'checkout\'); return false;">
