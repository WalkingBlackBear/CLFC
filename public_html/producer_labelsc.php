...
// If not producer_admin, then force producer to be the owner
...
$resultp = @mysql_query($sqlp, $connection) or die(debug_print ("ERROR: 786300 ", array ($sqlp,mysql_error()), basename(__FILE__).' LINE '.__LINE__));
...
while ( $row = mysql_fetch_array($resultpr) )
  {
    ...
    if ( $storage_code_prior != $row['storage_code'] || $member_id_prior != $member_id )
      {
        // Start a new label
        $route_code_info = &$row;
