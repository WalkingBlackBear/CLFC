...
if(strlen($search_string) > 0 && $action == "get_cust_prod")
  {
    ...
    $query = '
      ...
      FROM
        '.TABLE_PRODUCER.'
      WHERE
        producer_id = "'.mysql_real_escape_string ($search_string).'%"
        OR business_name LIKE "%'.mysql_real_escape_string ($search_string).'%"
      ORDER BY
        business_name,
        producer_id
      LIMIT 0,10';
    ...
    while ($row = mysql_fetch_object($result))
      {
        $row->business_name = htmlspecialchars ($row->business_name, ENT_QUOTES);
    ...
    $query = '
      ...
      FROM
        '.TABLE_MEMBER.'
      WHERE
        member_id = "'.mysql_real_escape_string ($search_string).'"
        OR first_name LIKE "'.mysql_real_escape_string ($search_string).'%"
        OR last_name LIKE "'.mysql_real_escape_string ($search_string).'%"
        OR first_name_2 LIKE "'.mysql_real_escape_string ($search_string).'%"
        OR last_name_2 LIKE "'.mysql_real_escape_string ($search_string).'%"
        OR business_name LIKE "%'.mysql_real_escape_string ($search_string).'%"
      ORDER BY
        last_name,
        last_name_2,
    ...
    while ($row = mysql_fetch_object($result))
      {
        ...
        $member_name = htmlspecialchars ($member_name, ENT_QUOTES);
