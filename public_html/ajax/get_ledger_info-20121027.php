...
//                             group_each_cost_with=[product|order]               //
...
if ($_REQUEST['group_each_cost_with'] == 'product') $summarize_by['each cost'] = 'product';
...
$css_trans_array = array (
  ...
  'each cost'     => 'ps', // Same key as "weight" -- treat them  as a single group
  'extra charge'  => 'ec', 
  'tax'           => 'tx'
  );
...
if ($_REQUEST['action'] == 'get_ledger_body')
  {
    ...
    // FIRST by delivery_id, and SECOND by product_id (which should be a a subset of transaction_group).
    ...
    $query = '
      SELECT
        ...
        '.NEW_TABLE_LEDGER.'.referenced_table,
        '.NEW_TABLE_LEDGER.'.referenced_key,
        ...
        COALESCE(baskets1.locked, baskets2.locked) AS locked,
    ...
    while ($number_of_rows > 1 && $row_count ++ <= $number_of_rows) // Cycles ONCE MORE THAN $number_of_rows
      {
        ...
        $transaction_current['referenced_table']    = $row['referenced_table'];
        $transaction_current['referenced_key']      = $row['referenced_key'];
        ...
        $text_key = $transaction_current['text_key'];
...
// Make sure everything still works if transactions are scattered in the database ordering
        ...
        if ($transaction_current['product_id'] &&
          ...
          {
            // Display the summary rows for each of the product summary items
            if (count (array_keys ($summarize_by, 'product')) > 0)
              {
                ...
                if ($product_summary_count == 1)
                ...
                else
                  {
                    ...
                    $response .= get_ledger_row_markup (array(
                      'unique_row_id' => 'gid_'.$transaction_prior['transaction_group'], // This is used for the CSS id
                      'display_class' => '',
                ...
                if ($order_summary_count[$this_text_key] == 1)
                  {
                    ...
                    $response .= str_replace ('{([T][B][D])}', $transaction_prior['balance'], $order_summary_singleton[$this_text_key]);
                  }
                elseif ($order_summary_count[$this_text_key] > 1)
                  {
                    ...
                    $response .= get_ledger_row_markup (array(
                      'unique_row_id' => $css_trans_array[$this_text_key].'id_'.$transaction_prior['transaction_group'], // This is used for the CSS id
                      'display_class' => '',
                      'display_scope' => $transaction_prior['basket_id'],
                      'timestamp' => '---',
                      'display_to_from' => '---',
        ...
        if ($summarize_by[$text_key] == 'product')
          {
            // Set to access the entire group by CSS with class = gidr_[transaction_group]
            // gidr is used for product-level groupings
            $transaction_current['display_class'] = 'gidr_'.$transaction_current['transaction_group'];
        ...
        elseif ($summarize_by[$text_key] == 'order')
          {
            // Set to access the entire group by CSS with class = [text_key_id]idr_[transaction_group]
