...
// If not an admin, then make sure the person is *themselves*
if (! valid_auth('member_admin,cashier'))
  {
    list($account_type, $account_id) = explode (':',$input_param['account_spec']);
    if ($account_type == 'member' && $account_id != $_SESSION['member_id']) exit (1);
    if ($account_type == 'producer' && $producer_id != $_SESSION['producer_id_you']) exit (1);
  }


// This file is a modification of get_ledger_info.php and is reorganized as a
// function instead of an ajax routine.
...
if ($_POST['action'] == 'get_ledger_head')
  {
...
if ($_POST['action'] == 'get_ledger_body')
  {
    $response = get_ledger_body($_POST);
    echo $response;
    exit (0);
  }

function get_ledger_body($input_param)
  {
    // Must be able to connect with the database
    global $connection;
    ...
    // $summary_type_array = array('bpid', 'pvid', 'basket_id', 'delcode_id', 'delivery_id');
    $summary_type_array = array();
    foreach (array('bpid', 'pvid', 'basket_id', 'delcode_id', 'delivery_id') as $summary_type)
      {
    ...
    $transaction_array['delcode_id'] = array();
    ...
    if ($input_param['delivery_id'])
      {
        $query_where = '
        AND ('.NEW_TABLE_LEDGER.'.delivery_id = "'.mysql_real_escape_string($input_param['delivery_id']).'"
        ...
        $query = '
          ...
          WHERE
            ...
            AND delivery_id < "'.mysql_real_escape_string($input_param['delivery_id']).'"';
    ...
    while ($row = mysql_fetch_array($result))
      {
        ...
        $display_balance = '';
          ...
          // target_key,              source_description,       timestamp,             delcode_id
        ...
        elseif ($row['delcode_id'])
          {
            $detail = $row['delcode_id'].' basket'; //<pre>'.print_r($row,true).'</pre>';
          }
...
function get_display_row($transaction_data, $summary_type, $running_total, $display_type)
  {
    ...
    if (! $transaction_data['locked'])
      {
        $row_click_script = '<img class="control" src="'.PATH.'grfx/edit_icon.png" onclick="row_click('.
...
function get_ledger_header_markup ()
  {
    $response = '
      <tr class="ledger_header">
        <th class="control"></th>
        <th class="scope">Invoice No.</th>
        <th class="timestamp">Date / Time</th>
        <th class="from_to">To / From</th>
        <th class="text_key">For</th>
        <th class="quantity">Qty</th>
        <th class="detail">Description</th>
        <th class="more_less"></th>
        <th class="amount">Amt.</th>
        <th class="balance">Bal.</th>
      </tr>';
    return ($response);
  }
