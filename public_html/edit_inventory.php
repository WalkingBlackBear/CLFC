...
<div style="width:95%;margin:auto;">
  ...
  <br>
  <form action="'.$_SERVER['PHP_SELF'].'?producer_id='.$producer_id.'" method="post">
  <table border="1" cellspacing="0" width="100%" id="inventory">
    <tr>
      <th class="left" width="40%">Description<div style="float:right; background-color:#eef;padding:1px 3px;font-size:80%;font-weight:normal;"><a href="'.$_SERVER['PHP_SELF'].'?'.($_REQUEST['target_inventory_id']? 'target_inventory_id='.$_REQUEST['target_inventory_id'].'&' : '').($_REQUEST['a']? 'a='.$_REQUEST['a'].'&' : '').'enable_edit=true">[Edit descriptions]</a></div></th>
    ...
    foreach (array_keys ($inventory_array) as $inventory_id)
      {
        ...
        <tr>
          ...
          <td class="center">'.($inventory_array[$inventory_id]['number_of_products'] == 0 ? '<input type="checkbox" name="delete-'.$inventory_id.'" value="true">' : '<input type="checkbox" name="delete-'.$inventory_id.'" value="true">' ).'</td>
        </tr>';
      }
  }
