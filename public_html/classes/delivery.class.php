    ...
    function printChangeDeliveryInfoForm($basket_id, $member_id)
      {
        ...
        /*if(Security::getRegisteredAuthType()!="route_admin" && Security::getRegisteredAuthType()!="producer_admin" && Security::getRegisteredMemberId()!=$member_id){
        ...
        $sql = '
          SELECT
            '.TABLE_BASKET_ALL.'.deltype AS user_deltype,
            '.TABLE_BASKET_ALL.'.*,
            '.TABLE_DELCODE.'.*,
            '.TABLE_ROUTE.'.*
          FROM
        ...
        //the form itself
        $display .= '
          <form action="'.SELF.'?&basket_id='.$basket_id.'" method="post">
            <table>
              <tr>
                <td>
        ...
        while ($row = mysql_fetch_array($result))
          {
            ...
            $display .= '
                    <option value="'.trim($row['delcode_id']).'" '.$selected.'>'.$row['delcode'].'</option>';
          }
        $display .= '
                  </select>
                </td>
    ...
    function changeUserDeliveryInfo()
      {
        ...
        $sql = '
          UPDATE
            '.TABLE_BASKET_ALL.' 
          SET
            transcharge= "'.mysql_real_escape_string ($transcharge).'",
