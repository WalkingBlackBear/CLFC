...
while ( $row = mysql_fetch_array($rs) )
  {
    ...
    $sqlp = '
      ...
      LEFT JOIN 
        '.TABLE_PRODUCT_PREV.' ON '.TABLE_PRODUCT_PREV.'.product_id = '.TABLE_PRODUCT_PREP.'.product_id
      WHERE
    ...
    while ( $row = mysql_fetch_array($resultp) )
      {
        ...
        $sql = '
          ...
          FROM
            ...
            '.TABLE_PRODUCT_PREV.'
          WHERE
            '.TABLE_PRODUCT_PREV.'.subcategory_id = "'.mysql_real_escape_string ($subcategory_id).'"
        ...
        while ( $row = mysql_fetch_array($result) )
          {
            ...
            while ( $current_product_id != $product_id )
              ...
              <tr>
                ...
                <td valign="top" align=center>'.$font2.' <strike>'.$price_display.'</strike></font></td>
...
<font size="+3">'.ucfirst (SITE_NAME).' '.$date_today.' List of Products Not Available Until Further Notice - Do not order in '.$this_month.'!!!</font>
<hr color="#000000" noshade size="2" width="100%">
'.$display;

$page_title_html = '<span class="title">Printable Product Lists</span>';
$page_subtitle_html = '<span class="subtitle">Unlisted Products</span>';
$page_title = 'Printable Product Lists: Unlisted Products';
