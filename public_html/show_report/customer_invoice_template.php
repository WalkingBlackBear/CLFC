...
function inventory_display_calc(&$product, &$unique)
  { 
    ...
    elseif ($product[$this_row]['out_of_stock'] == $product[$this_row]['basket_quantity'])
      $out_checkmark = '<img alt="out of stock" src="'.BASE_URL.PATH.'grfx/out.png" width="30">'; // Fully out of stock
    else
      $out_checkmark = '<img alt="out of stock" src="'.BASE_URL.PATH.'grfx/part.png" width="30">'; // Partly filled
...
function open_list_top(&$product, &$unique)
  {
    $list_top = '
        ...
        View as
          '.($unique['view'] != 'adjusted' ? '[<a href="'.$_SERVER['PHP_SELF'].'?'.($_GET['type'] ? 'type='.$_GET['type'] : '').($_GET['delivery_id'] ? '&delivery_id='.$_GET['delivery_id'] : '').($_GET['member_id'] ? '&member_id='.$_GET['member_id'] : '').'&view=adjusted">Adjusted</a>]': '').'
          '.($unique['view'] != 'original' ? '[<a href="'.$_SERVER['PHP_SELF'].'?'.($_GET['type'] ? 'type='.$_GET['type'] : '').($_GET['delivery_id'] ? '&delivery_id='.$_GET['delivery_id'] : '').($_GET['member_id'] ? '&member_id='.$_GET['member_id'] : '').'&view=original">Original</a>]': '').'
          '.(($unique['view'] != 'editable' && CurrentMember::auth_type('cashier') && $_GET['member_id'] != $member_id) ? '[<a href="'.$_SERVER['PHP_SELF'].'?'.($_GET['type'] ? 'type='.$_GET['type'] : '').($_GET['delivery_id'] ? '&delivery_id='.$_GET['delivery_id'] : '').($_GET['member_id'] ? '&member_id='.$_GET['member_id'] : '').'&view=editable">Editable</a>]': '').'
        invoice.
      </span>
          <table width="100%" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td align="left" valign="top"><!-- FOOTER LEFT "'.(strpos ($unique['auth_type'], 'institution') !== false ? $unique['business_name'] : '').$unique['last_name'].', '.$unique['first_name'].'" -->
                <font size="+2"><b>'.$unique['preferred_name'].' '.(strpos ($unique['auth_type'], 'institution') !== false ? $unique['business_name'].'<br></font>(attn: '.$unique['first_name'].' '.$unique['last_name'].')' : '</font>').'</b>
              </td>
              <td valign="top" align="right">
                <table border="0">
                  <tr>
                    <td align="center">
                      <img src="CLFC-Logo-2014.png" alt="logo" width="250">
                    </td>
                  </tr>
                  <tr>
                    <td align="center">
            ...
            <tr>
              <td colspan="2">
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    <td align="left">
                      <font size="+2">'.$unique['member_id'].'-'.$unique['delcode_id'].' ('.$unique['delcode'].')</font>
                    </td>
                    <td align="right">
...
($unique['deltype'] == 'H' || $unique['deltype'] == 'P' ? '
...
: '').'<br><br>
              ...
              <td valign="top"><strong>Delivery/pickup details:</strong>
                <dl>
                  <dt><font face="Times New Roman">'.$unique['delcode'].'</font></dt>
                  <dd><pre><font face="Times New Roman">'.$unique['deldesc'].'</font></pre></dd>
                </dl>
              </td>
            </tr>
            <tr>
...
: '').
// Override the invoice_price from the database and subtotal with fees ( || 1 )
'
<!-- NEED 7 -->'.
// '
//           <tr>
//             <td colspan="6" align="right"><br><b>SUBTOTAL</b></td>
//             <td align="right" width="8%"><br><b>$&nbsp;'.number_format($unique['total_order_amount'] - ($unique['invoice_price'] == 1 ? 0 : $unique['total_order_customer_fee']) - $unique['total_order_tax'], 2).'</b></td>
//           </tr>'.
// Override the invoice_price from the database and DO NOT SHOW the Fee ( && 0 )
($product[$this_row]['delivery_id'] >= DELIVERY_NO_PAYPAL && $unique['invoice_price'] == 0 && $unique['customer_fee_percent'] != 0 && 0 ? '
          <tr>
            <td colspan="6" align="right"><b>+ '.number_format($unique['customer_fee_percent'], 0).'% Fee</b></td>
            <td align="right" width="8%"><b>$&nbsp;'.number_format($unique['total_order_customer_fee'], 2).'</b></td>
          </tr>'
: '').
($unique['total_order_tax'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>* Sales tax</b></td>
            <td align="right" width="8%"><b>$ '.number_format($unique['total_order_tax'], 2).'</b></td>
          </tr>'
: '').
($product[$this_row]['exempt_adjustment_cost'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>Non-taxed Adjustments</b></td>
            <td  align="right" width="8%"><b>$&nbsp;'.number_format($product[$this_row]['exempt_adjustment_cost'], 2).'</b></font></td>
          </tr>'
: '').
($adjustment['total_delivery_cost'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>Extra Charge for Delivery </b></font></td>
            <td  align="right" width="8%"><b>$&nbsp;'.number_format($adjustment['total_delivery_cost'], 2).'</b></td>
          </tr>'
: '').'
          <tr>
          ...
          <tr>
            <td colspan="6" align="right"><b>Invoice&nbsp;Total </b></td>
            <td align="right" width="8%"><b>$&nbsp;'.number_format($unique['total_order_amount'], 2).'</b></td>
          </tr>'.
($unique['balance_forward'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>Previous '.($unique['balance_forward'] < 0 ? 'Credit' : 'Balance Due').'</b></td>
            <td align="right" width="8%"><b>$&nbsp;'.number_format($unique['balance_forward'], 2).'</b></td>
          </tr>'
: '').
$unique['included_adjustments'].'
          <tr>
            <td colspan="5" height="1"></td>
            <td colspan="2" height="1" background="'.BASE_URL.PATH.'grfx/black_pixel.gif"></td>
          </tr>
          <tr>
            <td colspan="6" align="right"><font size="+2">PLEASE PAY:&nbsp;</font></td>
            <td align="right"><font size="+2">'.($product[$this_row]['unfilled_random_weight'] ? '<font size="-1">'.$product[$this_row]['display_weight_pending_text'].'</font>' : '$&nbsp;'.number_format ($unique['total_order_amount'] + $unique['balance_forward'] + $unique['included_adjustment_total'], 2)).'</font></td>
          </tr>'.
// ADJUSTMENT DISPLAY
$unique['excluded_adjustments'].
(round ($product[$this_row]['most_recent_payment_amount'], 2) > 0 ? '
          <tr>
            <td colspan="6" align="right">Thank you for your most recent payment of $&nbsp;'.number_format ($product[$this_row]['most_recent_payment_amount'], 2).'.</td>
            <td></td>
          </tr>'
: '').
($_GET['output'] != 'pdf' ? '
          <tr id="payment_options">
            <td colspan="7" align="right">
              <table width="60%" border="0" cellspacing="10" align="center" style="margin-top:2em;">
                <tr>
                ...
                <tr>
                  <td colspan="2" valign="top">
                    <ul>
                      <li>Pay $&nbsp;'.number_format ($unique['total_order_amount'] + $unique['balance_forward'] + $unique['included_adjustment_total'], 2).' by cash or check at order pickup</li>
                        <span style="padding-left:30px; line-height:2em;">&mdash; OR &mdash;</span>
                      <li>Complete an electronic money transfer (EMT) in advance. Please bring transfer confirmation with you to pick up site.</li>
                    </ul>
                  </td>
                </tr>'.
// Disable the paypal option ( && 0 )
(PAYPAL_EMAIL  && 0 ? '
                <tr>
                  <td colspan="2">OR</td>
                </tr>
                <tr>
                  ...
                  <td valign="top">
                    <b>Pay $&nbsp;<input type="text" name="amount" value="'.number_format ($unique['total_order_amount'] + $unique['balance_forward'] + $unique['included_adjustment_total'], 2).'" style="border:0;" size="'.strlen(number_format ($unique['total_order_amount'] + $adjustment['total_delivery_cost'] + $unique['order_cost'] + $unique['delivery_cost'] + $unique['balance_forward'], 2)).'"> now with PayPal</b><br>
                  ...
                  <td valign="top">
                    ...
                    <input type="hidden" name="item_name" value="Food Coop: '.$unique['preferred_name'].' #$member_id Delivery Date: '.$unique['delivery_date'].'">
...
function show_product_row(&$product, &$unique)
  {
    ...
    if ($product[$this_row]['adjustment_group_memo'] != "")
      {
        ...
        $unique['adjustment_markup'] .= '
          <tr align="center" class="'.$adjustment_class.'">
            <td colspan="2"></td>
            <td colspan="5" align="left">Adjustment: '.$product[$this_row]['adjustment_group_memo'].'</td>
          </tr>';
        debug_print ("INFO ", $product, basename(__FILE__).' LINE '.__LINE__);
        }
    // Aggregate the total order cost over the whole order
    $unique['total_order_amount'] += $product[$this_row]['amount'];
    // Override the invoice_price setting and ALWAYS show the customer price ( || 1)
    if ($unique['invoice_price'] == 1 || 1)
    ...
    if ($product[$this_row + 1]['product_id'] != $product[$this_row]['product_id'] ||
        $product['this_row'] == $product['number_of_rows'])
      {
        ...
        $display_line = '
          <tr align="center" class="'.$adjustment_class.'">
            <td width="40" align="right" valign="top">'.($unique['view'] == 'editable' ? '<img src="'.PATH.'grfx/edit_icon.png" onclick="popup_src(\'adjust_ledger.php?type=product&target='.$product[$this_row]['bpid'].'\');">' : '').'</td>
            ...
            <td width="13" align="right" valign="top"><b>'.$tax_display.'$'.number_format($product['total_product_amount'] - $product['total_product_tax'], 2).'</b></td>
          </tr>'.
          // Show adjustment comment
          ($unique['adjustment_markup'] != "" ? $unique['adjustment_markup'] : '');
...
function show_adjustment_row(&$adjustment, &$unique)
  {
    ...
    // in the totals.
    if ($adjustment[$this_row]['text_key'] == 'payment received' && $adjustment[$this_row]['delivery_id'] == $unique['delivery_id'])
      {
            ...
            $unique['excluded_adjustments'] .= '
              <tr>
                ...
                <td colspan="5" align="right">'.
                  ...
                  <img src="'.PATH.'grfx/edit_icon.png" onclick="popup_src(\'adjust_ledger.php?type=single&target='.$adjustment[$this_row]['transaction_id'].'\');">' : '').'
