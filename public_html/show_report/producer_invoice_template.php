...
function pricing_display_calc(&$product, &$unique)
  { 
    ...
    if (SHOW_ACTUAL_PRICE) $unit_price = $product[$this_row]['unit_price'] * (1 + ($markup / 100));
    else $unit_price = $product[$this_row]['unit_price'];
...
function open_list_top(&$product, &$unique)
  {
    $display_list_top = '
          <table width="100%" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td align="left" valign="top"><!-- FOOTER LEFT "'.$unique['business_name'].'" -->
                <font size="+2"><b>'.$unique['business_name'].'</b>
              </td>
...
: '').'
              ...
              <td valign="bottom" align="right" style="vertical-align:bottom;">
                <font size="+2">'.date ("F j, Y", strtotime ($unique['delivery_date'])).'
              </td>
            </tr>
            <tr>
              <td colspan="2">
...
: '').'
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          ...
          <tr>
            ...
            <th valign="bottom" bgcolor="#444444" align=right width="8%"><font color="#ffffff" size="-1">Total</font></th>
          </tr>';
...
<!-- NEED 7 -->
          <tr>
            ...
            <td align="right" width="8%"><br><b>$&nbsp;'.number_format($unique['total_order_amount'] - $unique['total_order_customer_fee'] - $unique['total_order_tax'], 2).'</b></td>
          </tr>'.
($product[$this_row]['delivery_id'] >= DELIVERY_NO_PAYPAL && $unique['invoice_price'] == 0 && $unique['producer_fee_percent'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>- '.number_format($unique['producer_fee_percent'], 0).'% Fee</b></td>
            <td align="right" width="8%"><b>$&nbsp;'.number_format($unique['total_order_fee'], 2).'</b></td>
          </tr>'
: '').
($product[$this_row]['exempt_adjustment_cost'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>Non-taxed Adjustments</b></td>
            <td  align="right" width="8%"><b>$&nbsp;'.number_format($product[$this_row]['exempt_adjustment_cost'], 2).'</b></td>
          </tr>'
: '').'
          <tr>
          ...
          <tr>
            ...
            <td align="right" width="8%"><b>$&nbsp;'.number_format($unique['total_order_amount'] - $unique['total_order_fee'], 2).'</b></td>
          </tr>'.
...
(round($product[$this_row]['membership_cost'], 2) != 0 || $product[$this_row]['order_cost'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>Membership/Fees</b></td>
            <td  align="right" width="8%"><b>$ '.number_format($product[$this_row]['membership_cost'] + $product[$this_row]['order_cost'], 2).'</b></td>
          </tr>'
: '').
($product[$this_row]['previous_balance'] != 0 ? '
          <tr>
            <td colspan="6" align="right"><b>(Our records show a previous '.($product[$this_row]['previous_balance'] < 0 ? 'Credit' : 'Balance Due').'</b></td>
            <td align="right" width="8%"><b>$&nbsp;'.number_format($product[$this_row]['previous_balance'], 2).')</b></td>
          </tr>'
: '').'
          <tr>
            <td colspan="6" height="1"></td>
          ...
          <tr>
            <td colspan="5" align="right"><font size="+2"> NET:</td>
            <td colspan="2" align="right"><font size="+2">$&nbsp;'.number_format ($unique['total_order_amount'] - $unique['total_order_fee'] + $unique['balance_forward'], 2).'</td>
          </tr>
        </table>';
      }
    ...
    switch ($unique['major_product'])
      {
        ...
        case 'product_id':
          ...
          <tr>
            ...
            <td colspan="5"><font face="arial" color="#770000" size="-1"><b>'.$product[$this_row]['product_name'].'</b> &ndash; '.$product[$this_row]['pricing_display'].'</td>
          </tr>';
        case 'producer_name':
          break;
        // Otherwise...
...
function major_product_close (&$product, &$unique)
  {
    ...
    '
          <tr align="left">
            ...
            <td align="right"><b>$'.number_format($unique['total_product_amount'], 2).'</b></td>
          </tr>';
...
function show_product_row(&$product, &$unique)
  {
    ...
    if ($product[$this_row + 1]['bpid'] != $product[$this_row]['bpid'] ||
        $product['this_row'] == $product['number_of_rows'])
      {
        ...
        $display_line = '
          <tr align="center">
            <td width="40" align="right" valign="top">'.$product[$this_row]['inventory_display'].$product[$this_row]['customer_fee_total'].'</td>
            <td width="50" align="right" valign="top">'.$product[$this_row]['member_id'].'&nbsp;&nbsp;</td>
            ...
            <td width="13" align="right" valign="top">'.number_format($unique['total_bpid_amount'], 2).'</td>
