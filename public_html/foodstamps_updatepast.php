...
while ( $row = mysql_fetch_array($result1) )
  {
    ...
    $sql2 = '
      UPDATE
        '.TABLE_BASKET.'
      SET
        retail_staple = "'.mysql_real_escape_string ($retail_staple).'"
      WHERE
...
$sql = '
  ...
  LEFT JOIN
    '.TABLE_ORDER_CYCLES.' ON '.TABLE_BASKET_ALL.'.delivery_id = '.TABLE_ORDER_CYCLES.'.delivery_id
  WHERE
    '.TABLE_BASKET_ALL.'.delivery_id = "'.mysql_real_escape_string (ActiveCycle::delivery_id()).'"
    AND '.TABLE_BASKET.'.future_delivery_id != "'.mysql_real_escape_string (ActiveCycle::delivery_id()).'"
  ORDER BY
...
$page_subtitle_html = '<span class="subtitle">Staple/Retail/Non-food Totals</span>';
$page_title = 'Food Stamps: Staple/Retail/Non-food Totals';
