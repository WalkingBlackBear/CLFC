...
if ( $add_ziptoroute == 'yes' )
  {
    if ( $deltype == 'P' )
      {
        $updateq = 'delp = "'.mysql_real_escape_string ($delcode_id).'"';
      }
    elseif ( $deltype == 'D' )
      {
        $updateq = 'deld = "'.mysql_real_escape_string ($delcode_id).'"';
      }
    $sqlu2 = '
      UPDATE
        '.TABLE_ZIP.'
...
while ( $row = mysql_fetch_array($rsr) )
  {
    ...
    $display .= '<tr bgcolor="#AEDE86"><td id="'.$route_id.'" colspan="2" align="left">
    ...
    $sqlr2 = '
      ...
      FROM
        '.TABLE_DELCODE.'
      WHERE
        '.TABLE_DELCODE.'.route_id = "'.mysql_real_escape_string ($route_id).'"
      GROUP BY
    ...
    while ( $row = mysql_fetch_array($rsr2) )
      ...
      $sqlr3 = '
        ...
        FROM
          '.TABLE_ZIP.'
        WHERE
          '.TABLE_ZIP.'.delp = "'.mysql_real_escape_string ($delcode_id).'"
      ...
      $sqlr4 = '
        ...
        WHERE
          (
            '.TABLE_ZIP.'.delp = "'.mysql_real_escape_string ($delcode_id).'"
            OR '.TABLE_ZIP.'.deld = "'.mysql_real_escape_string ($delcode_id).'"
          )
      ...
      $display .= '
        <tr bgcolor="#CCCCCC"><td colspan="2" align="left" id="'.$delcode_id.'">
  ...
  <table width="685" cellpadding="7" cellspacing="2" border="0">
    ...
    <tr>
      <td colspan="2" align="left" bgcolor="#EEEEEE">
        <ul>
          Click to view the full public <a href="'.LOCATIONS_PAGE.'" target="_blank">list of locations</a>.
          <br><br>
...
$page_subtitle_html = '<span class="subtitle">Members on Your Route</span>';
$page_title = 'Route Information: Members on Your Route';
