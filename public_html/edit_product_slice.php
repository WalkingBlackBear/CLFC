    ...
    if ($row->subcategory_id != $subcategory_id_prior)
      ...
      <tr class="subcat_head">
        <td colspan="4">
          ...
          <span class="next_link">[ <a href="'.$_SERVER['PHP_SELF'].'?slice_subcategory='.$row->subcategory_id.'">View all '.$row->subcategory_name.' products</a> ]
        </td>
        <td colspan="6">
          <form action="'.$_SERVER['PHP_SELF'].'?'.$this_slice.'" method="post">
    ...
    if ($row->producer_id != $producer_id_prior)
      ...
      <tr class="producer_head">
        <td colspan="4">
          ...
          <span class="next_link">[ <a href="'.$_SERVER['PHP_SELF'].'?slice_producer='.$row->producer_id.'">View all '.$row->business_name.' products</a> ]
        </td>
        <td colspan="6">
          <form action="'.$_SERVER['PHP_SELF'].'?'.$this_slice.'" method="post">
