...
function get_producer ($producer_id)
  {
    ...
    $producer_fields = array (
      ...
      'pub_fax',
      'pub_web',
      // available_delcode_ids <----- also included with this information: e.g. 'GRNDI,LNC,OMA'
      );
    $query = '
      SELECT
        '.implode (",\n        ", $producer_fields).',
        (SELECT GROUP_CONCAT(delcode_id)
         FROM availability
         WHERE producer_id = '.TABLE_PRODUCER.'.producer_id) AS available_delcode_ids
      FROM '.TABLE_PRODUCER.'
