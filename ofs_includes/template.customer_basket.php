        ...
        ($data['availability'] == true ?
          ($data['basket_quantity'] > 0 || !$data['inventory_id'] || $data['inventory_quantity'] > 0 ?
            ...
            '<form action="'.$_SERVER['PHP_SELF'].'?type='.$_GET['type'].'#X'.$data['product_id'].'" method="post">
               <input id="add'.$data['product_id'].'" class="basket_add" type="image" name="basket_add" src="'.PATH.'/grfx/basket_add.png" width="24" height="24" border="0" alt="Submit" onclick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'add\'); return false;" '.($data['basket_quantity'] > 0 ? ($data['inventory_id'] && $data['inventory_quantity'] == 0 ? 'style="display:none;"' : '') : 'style="display:none;"').'>
               <input id="sub'.$data['product_id'].'" class="basket_sub" type="image" name="basket_sub" src="'.PATH.'/grfx/basket_sub.png" width="24" height="24" border="0" alt="Submit" onclick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'sub\'); return false;" '.($data['basket_quantity'] > 0 ? '' : 'style="display:none;"').'>
               ...
               <input id="basket_empty'.$data['product_id'].'" class="basket" type="image" name="basket" src="'.PATH.'/grfx/basket-egi_add.png" width="48" height="48" border="0" alt="Submit" onClick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'add\'); return false;" '.($data['basket_quantity'] > 0 ? 'style="display:none;"' : '').'>
               <img id="basket_full'.$data['product_id'].'" class="basket" src="'.PATH.'/grfx/basket-fcs.png" width="48" height="48" border="0" '.($data['basket_quantity'] > 0 ? '' : 'style="display:none;"').'>
               </div>
             </form>
...
($_SESSION['member_id'] == 85 ? '
      <td>
        ...
        ($data['delcode_id'] && $data['delivery_postal_code'] ?
          ($data['checked_out'] ? '
            <img class="checkout_check" src="'.PATH.'/grfx/checkout-ccs.png">
            <span class="checkout_text">Ordered!</span>'
            : '
            <input type="image" class="checkout_check" src="'.PATH.'/grfx/checkout-g.png" onclick="AddToCart('.$data['product_id'].','.$data['product_version'].',\'checkout\'); return false;">
