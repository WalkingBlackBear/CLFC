...
class ActiveCycle
  {
    ...
    private function get_active_delivery_info ($new_delivery_id)
      {
...
class CurrentBasket
  {
    ...
    private static $delcode_id = false;
    private static $delcode = false;
        ...
        if (self::$query_complete === false)
          {
            ...
            $query = '
              ...
              FROM
                '.NEW_TABLE_BASKETS.'
              LEFT JOIN '.TABLE_DELCODE.' ON '.TABLE_DELCODE.'.delcode_id = '.NEW_TABLE_BASKETS.'.delcode_id
              WHERE
            ...
            if ($row = mysql_fetch_object ($result))
              {
                ...
                self::$delcode_id = $row->delcode_id;
                self::$delcode = $row->delcode;
    ...
    public static function delcode_id ()
      {
        self::get_basket_info ();
        return self::$delcode_id;
      }
...
class CurrentMember
  {
    ...
    public function auth_type ($test_auth)
      {
        self::get_member_info ();
